...
class GeneSetResult:
    ...
    def __init__(self, 
        ...
        self.null_median_exp_A: Optional[np.ndarray] = None
        self.null_median_exp_B: Optional[np.ndarray] = None
        self.null_projections: Optional[np.ndarray] = None
        self.q_value: Optional[float] = None
        self.med_exp_q_value_A: Optional[float] = None
        self.med_exp_q_value_B: Optional[float] = None
        self.non_adj_p: Optional[float] = None
        self.non_adj_med_exp_p_A: Optional[float] = None
        self.non_adj_med_exp_p_B: Optional[float] = None
        self.test_l1: Optional[float] = None
        self.test_median_exp_A: Optional[float] = None
        self.test_median_exp_B: Optional[float] = None
...
class ROMA:
    ...
    def __init__(self) -> None:
        ...
        self.test_median_exp_A: Optional[float] = None
        self.test_median_exp_B: Optional[float] = None
        self.med_exp_p_value_A: Optional[float] = None
        self.med_exp_p_value_B: Optional[float] = None
        self.projections_1: Optional[np.ndarray] = None
        self.projections_2: Optional[np.ndarray] = None
        ...
        self.q_Med_Exp_A_threshold: float = 0.05
        self.q_Med_Exp_B_threshold: float = 0.05
    ...
    def loocv(self, subset: sc.AnnData, multiomics: bool = False, verbose: int = 0, for_randomset: bool = False) -> List[int]:
        ...
        if multiomics:
            X_df=self.anndata_to_multiindex_df(self.subset, feature_col="feature type").T.copy() #to have the right type for GSVD
        X = np.asarray(X)
        if n_samples < 2:
            if verbose:
        ...
        for i, (train_index, _) in enumerate(loo.split(X)):
            if multiomics:
                ...
                A=vals.loc["prot"].to_numpy()
                B=vals.loc["transcri"].to_numpy()
                #A_centered=A-A.mean(axis=0)
                #B_centered=B-B.mean(axis=0)
                D1, D2, Y, R, U, V = pygsvd.gsvd(A, B)
                svd=GSVD_wrapper(D1, D2, Y, R, U, V, n_components=1)
            else:
    ...
    def robustGSVD(self, 
        ...
        # Perform GSVD
        A,B=[X_df.loc["prot"].to_numpy(), X_df.loc["transcri"].to_numpy()] #[((X_df.T)["prot"].T).to_numpy(), ((X_df.T)["transcri"].T).to_numpy()] 
        D1,D2,Y,R,U,V  = pygsvd.gsvd(A, B) 

        svd_ = GSVD_wrapper(D1,D2,Y,R,U,V, n_components=2)
    ...
    def compute_median_exp(self, 
        ...
        # Compute projections
        projA_1=X.loc["prot"] @ pc1
        projB_1=X.loc["transcri"] @ pc1
        
        projA_2=X.loc["prot"] @ pc2
        projB_2=X.loc["transcri"] @ pc2

        projections_1 = pd.concat([projA_1, projB_1], axis=0, keys=["prot", "transcri"])
        projections_2 = pd.concat([projA_2, projB_2], axis=0, keys=["prot", "transcri"])
        ...
        median_exp_A = np.median(projA_1)
        median_exp_B = np.median(projB_1)

        return median_exp_A, median_exp_B, projections_1, projections_2
    ...
    def process_iteration(self, 
        ...
        if multiomics:
            svd_, X = self.robustGSVD(self.adata, gene_subset, outliers, 
                                        for_randomset=True)
        elif incremental:
        ...
        median_exp_A, median_exp_B, null_projections_1, null_projections_2 = self.compute_median_exp(
            svd_, X, raw_X_subset
        )
        
        return l1, median_exp_A, median_exp_B, null_projections_1, null_projections_2
    ...
    def randomset_parallel(self, 
        ...
        if candidate_nullgeneset_size in self.null_distributions:
            self.nulll1, self.null_median_exp_A, self.null_median_exp_B = self.null_distributions[candidate_nullgeneset_size]
            if verbose:
        ...
        nulll1, null_median_exp_A, null_median_exp_B, null_proj1, null_proj2 = zip(*results)
        
        nulll1_array = np.array(nulll1)
        null_median_exp_array_A = np.array(null_median_exp_A)
        null_median_exp_array_B = np.array(null_median_exp_B)
        
        # Store in cache
        self.null_distributions[candidate_nullgeneset_size] = [
            nulll1_array.reshape(-1, 1),  # Ensure 2D array
            null_median_exp_array_A,
            null_median_exp_array_B,
        ]
        
        self.nulll1 = nulll1_array.reshape(-1, 1)
        self.null_median_exp_A = null_median_exp_array_A
        self.null_median_exp_B = null_median_exp_array_B
    ...
    def get_raw_p_values(self, gene_set_name: Optional[str] = None) -> None:
        ...
        if not hasattr(self, 'nulll1') or self.nulll1 is None or len(self.nulll1) == 0:
            ...
            test_median_exp_A, test_median_exp_B, projections_1, projections_2 = self.compute_median_exp(
                self.svd, self.X, self.raw_X_subset, gene_set_name
            )
            
            self.test_median_exp_A = test_median_exp_A
            self.test_median_exp_B = test_median_exp_B
        ...
        null_median_distribution_A = np.array(self.null_median_exp_A)
        null_median_distribution_B = np.array(self.null_median_exp_B)
        ...
        # Empirical p-value
        med_exp_p_value_A = (np.sum(np.abs(null_median_distribution_A) >= np.abs(test_median_exp_A)) + 1) / (len(null_median_distribution_A) + 1)
        med_exp_p_value_B = (np.sum(np.abs(null_median_distribution_B) >= np.abs(test_median_exp_B)) + 1) / (len(null_median_distribution_B) + 1)
        
        self.test_median_exp_A = test_median_exp_A
        self.test_median_exp_B = test_median_exp_B
        self.med_exp_p_value_A = med_exp_p_value_A
        self.med_exp_p_value_B = med_exp_p_value_B
        self.projections_1 = projections_1
        self.projections_2 = projections_2
        
        # Clear temporary data
        self.nulll1 = None
        self.null_median_exp_A = None
        self.null_median_exp_B = None
    ...
    def assess_significance(self, results: Dict[str, GeneSetResult]) -> Dict[str, GeneSetResult]:
        ...
        n_tests = len(results)
        ps = np.zeros(n_tests)
        med_ps_A = np.zeros(n_tests)
        med_ps_B = np.zeros(n_tests)
        
        # Collect p-values
        for i, (_, gene_set_result) in enumerate(results.items()):
            ps[i] = gene_set_result.non_adj_p
            med_ps_A[i] = gene_set_result.non_adj_med_exp_p_A
            med_ps_B[i] = gene_set_result.non_adj_med_exp_p_B
        
        # Apply Benjamini-Hochberg correction
        qs = benj_hoch(ps)
        med_exp_qs_A = benj_hoch(med_ps_A)
        med_exp_qs_B = benj_hoch(med_ps_B)
        
        # Update results
        for i, (_, gene_set_result) in enumerate(results.items()):
            gene_set_result.q_value = qs[i]
            gene_set_result.med_exp_q_value_A = med_exp_qs_A[i]
            gene_set_result.med_exp_q_value_B = med_exp_qs_B[i]
    ...
    def p_values_in_frame(self, assessed_results: Dict[str, GeneSetResult]) -> pd.DataFrame:
        ...
        for name, result in assessed_results.items():
            ...
            data['Median Exp A'][name] = result.test_median_exp_A
            data['ppv Med Exp A'][name] = result.non_adj_med_exp_p_A
            data['Median Exp B'][name] = result.test_median_exp_B
            data['ppv Med Exp B'][name] = result.non_adj_med_exp_p_B
            data['q L1'][name] = result.q_value
            data['q Med Exp A'][name] = result.med_exp_q_value_A
            data['q Med Exp B'][name] = result.med_exp_q_value_B
    ...
    def compute(self, 
        ...
        for gene_set_name in sorted_gene_sets:
            ...
            gene_set_result.non_adj_med_exp_p_A = self.med_exp_p_value_A
            gene_set_result.non_adj_med_exp_p_B = self.med_exp_p_value_B
            gene_set_result.test_median_exp_A = self.test_median_exp_A
            gene_set_result.test_median_exp_B = self.test_median_exp_B
        ...
        self.select_active_modules(self.q_L1_threshold, self.q_Med_Exp_A_threshold, self.q_Med_Exp_B_threshold)
    ...
    def select_active_modules(self, 
                            ...
                            q_Med_Exp_A_threshold: float = 0.05,
                            q_Med_Exp_B_threshold: float = 0.05) -> None:
        """
        ...
        active_modules = df[(df['q L1'] <= q_L1_threshold) | (df['q Med Exp A'] <= q_Med_Exp_A_threshold) | (df['q Med Exp B'] <= q_Med_Exp_B_threshold)]
    ...
    def summary(self) -> None:
        ...
        print(f"Active modules (shifted in A): {len(active[active['q Med Exp A'] <= self.q_Med_Exp_A_threshold])}")
        print(f"Active modules (shifted in B): {len(active[active['q Med Exp B'] <= self.q_Med_Exp_B_threshold])}")
        ...
        print(f"\n{color.BOLD}Top 5 shifted modules for A:{color.END}")
        top_med = stats.nsmallest(5, 'q Med Exp A')[['Median Exp A', 'q Med Exp A']]
        for idx, row in top_med.iterrows():
            print(f"  {idx}: Med={row['Median Exp A']:.3f}, q={row['q Med Exp A']:.3e}")

        # Top modules by median expression
        print(f"\n{color.BOLD}Top 5 shifted modules for B:{color.END}")
        top_med = stats.nsmallest(5, 'q Med Exp B')[['Median Exp B', 'q Med Exp B']]
        for idx, row in top_med.iterrows():
            print(f"  {idx}: Med={row['Median Exp B']:.3f}, q={row['q Med Exp B']:.3e}")
