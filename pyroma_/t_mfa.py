...
from prince_pca import PCA


class t_MFA(PCA, collections.UserDict):
    def __init__(
        self,
        n_components=2,
        n_iter=3,
        ...
        super().__init__(
            rescale_with_mean=False, #True,
            rescale_with_std=False, #True,
    ...
    def fit(self, Xt, y=None, groups=None, supplementary_groups=None):
        #First of all, transposing entry matrix for the first analysis
        X=Xt.T
        ...
        if self.which_mfa=="frobnorm":
            Z = self._build_Z(X)
            row_weights = np.array(
                [
                    1 / (np.linalg.norm(Z, ord="fro")**2)
        ...
        super().fit(
            Z.T,                                   #transposing eveything back again to perform MFA
