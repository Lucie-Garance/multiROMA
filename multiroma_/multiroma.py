...
sys.path.insert(0, "/Users/lucie-garance/pygsvd")
import pygsvd
from .t_mfa import t_MFA  #t_mfa
...
from .sparse_methods import * #.sparse_methods
from .utils import anndata_to_multiindex_df, mudata_to_flat_anndata_v2, flat_anndata_to_mudata_v2 #utils
...
class MFA_wrapper:
    ...
    def __init__(self, 
        ...
        v1,v2=list(adata.var["modality"].unique())
        v1_variance_ratio=mfa[v1].eigenvalues_ / mfa[v1].total_inertia_
        v2_variance_ratio=mfa[v2].eigenvalues_ / mfa[v2].total_inertia_
        self.explained_variance_ratio_=np.mean([v1_variance_ratio,v2_variance_ratio], axis=0)
        self.components_ = mfa.column_coordinates_.to_numpy().T

#Class object to wrap the GSVD results and get the right attributes
class GSVD_wrapper:
    """
    Wrapper for GSVD:
    ...
    ------------
    explained_variance_ratio_ : np.array
        ratio of explained variance, from GSVD eigenvalues
    components_ : np.array
        first factors (equivalent of PC1/PC2 in PCA)
    """
    def __init__(self, 
        ...
        self.explained_variance_ratio_ = [np.sum(R[i, :]**2)/(np.sum(R**2)) for i in range(n_components)]
        self.components_ = Y[:, :n_components].T
        

class GeneSetResult_MFA:
    """
    Container for gene set analysis results of MFA.
...
class GeneSetResult_GSVD:
    """
    Container for gene set analysis results of GSVD.
    ...
    nulll1 : np.ndarray
        Null distribution of L1 values
    null_median_exp_A : np.ndarray
        Null distribution of median expression – matrix A
    null_median_exp_B : np.ndarray
        Null distribution of median expression – matrix B
    null_projections : np.ndarray
        Null projections for both components
    q_value : float
        Adjusted p-value for L1 statistic
    med_exp_q_value_A : float
        Adjusted p-value for median expression – A projection
    med_exp_q_value_B : float
        Adjusted p-value for median expression – B projection
    non_adj_p : float
        Raw p-value for L1 statistic
    non_adj_med_exp_p_A : float
        Raw p-value for median expression – A projection
    non_adj_med_exp_p_B : float
        Raw p-value for median expression – B projection
    test_l1 : float
        L1 test statistic
    test_median_exp_A : float
        Median expression test statistic
    test_median_exp_B : float
        Median expression test statistic
    """
    ...
    def __init__(self, 
        ...
        self.projections_2: Optional[np.ndarray] = None
        self.nulll1: Optional[np.ndarray] = None
        self.null_median_exp_M1: Optional[np.ndarray] = None
        self.null_median_exp_M2: Optional[np.ndarray] = None
        self.null_projections: Optional[np.ndarray] = None
        self.q_value: Optional[float] = None
        self.med_exp_q_value_M1: Optional[float] = None
        self.med_exp_q_value_M2: Optional[float] = None
        self.non_adj_p: Optional[float] = None
        self.non_adj_med_exp_p_M1: Optional[float] = None
        self.non_adj_med_exp_p_M2: Optional[float] = None
        self.test_l1: Optional[float] = None
        self.test_median_exp_M1: Optional[float] = None
        self.test_median_exp_M2: Optional[float] = None
        self.custom_name: str = ""
...
class ROMA_MFA:
    ...
    def __init__(self) -> None:
        ...
        self.projections_1: Optional[np.ndarray] = None
        self.projections_2: Optional[np.ndarray] = None
        self.results: Dict[str, GeneSetResult_MFA] = {}
    ...
    def loocv(self, subset: sc.AnnData, verbose: int = 0, for_randomset: bool = False) -> List[int]:
        ...
        if self.omic=='multiomic':
            X_df=anndata_to_multiindex_df(self.subset, feature_col="modality").T.copy() #to have the right type for MFA
        X = np.asarray(X)
        if n_samples < 2:
            if verbose:
        ...
        for i, (train_index, _) in enumerate(loo.split(X)):
            if self.omic=='multiomic':
                ...
                svd=MFA_wrapper(mfa, self.adata)
            else:
    ...
    def robustMFA(self, 
        ...
        # Extract data for robust gene subset
        X_df=anndata_to_multiindex_df(adata[:, robust_gene_subset], feature_col="modality").copy()
        ...
        svd_ = MFA_wrapper(mfa, self.adata)
    ...
    def assess_significance(self, results: Dict[str, GeneSetResult_MFA]) -> Dict[str, GeneSetResult_MFA]:
        """
        Compute adjusted p-values using Benjamini-Hochberg procedure.
        
        Parameters
        ----------
        results : Dict[str, GeneSetResult_MFA]
    ...
    def p_values_in_frame(self, assessed_results: Dict[str, GeneSetResult_MFA]) -> pd.DataFrame:
        """
        Create DataFrame with all p-values and statistics.
        
        Parameters
        ----------
        assessed_results : Dict[str, GeneSetResult_MFA]
    ...
    def compute(self, 
        ...
        omic : str, default="multiomic"
            Data to integrate to ROMA
        iters : int, default=100
        ...
        if self.omic=='multiomic':
            self.adata = mudata_to_flat_anndata_v2(mudata, feature_col="modality")
        else:
            self.adata = mudata[self.omic]
        ...
        for gene_set_name in sorted_gene_sets:
            ...
            # Store results
            gene_set_result = GeneSetResult_MFA(
                self.subset, 
                self.subsetlist, 
                self.outliers, 
            ...
            gene_set_result.custom_name = f"GeneSetResult_MFA {gene_set_name}"
        ...
        # Store results
        self.mudata=flat_anndata_to_mudata_v2(self.adata, feature_col="modality")
        self.mudata.uns['ROMA'] = assessed_results
        self.mudata.uns['ROMA_stats'] = self.p_values_in_frame(assessed_results)
    ...
    def save_active_modules_results(self, 
        ...
        save_mudata : bool, default=True
            Whether to save the AnnData object
        """
        os.makedirs(path, exist_ok=True)
        
        active_modules = self.mudata.uns['ROMA_active_modules'].index
        
        if only_active:
            selected_dict = {k: v for k, v in self.mudata.uns['ROMA'].items() if k in active_modules}
        else:
            selected_dict = self.mudata.uns['ROMA']
        ...
        # Save DataFrames
        self.mudata.uns['ROMA_stats'].to_csv(os.path.join(path, "ROMA_stats.csv"))
        self.mudata.uns['ROMA_active_modules'].to_csv(os.path.join(path, "ROMA_active_modules.csv"))
        
        # Save AnnData if requested
        if save_mudata:
            del self.mudata.uns['ROMA']  # Remove large dictionary before saving
            self.mudata.write(os.path.join(path, "roma_mudata.h5mu"))
    ...
    def load_active_modules_results(self, path: str) -> None:
        ...
        for key in os.listdir(path):
            ...
            if os.path.isdir(key_dir):
                gene_set = GeneSetResult_MFA(None, None, None, None, None)
        ...
        if self.mudata is None:
            mudata_path = os.path.join(path, "roma_mudata.h5mu")
            if os.path.exists(mudata_path):
                self.mudata = mu.read_h5mu(mudata_path)
            else:
                raise FileNotFoundError(f"MuData file not found: {mudata_path}")
        
        # Load DataFrames
        self.mudata.uns['ROMA'] = loaded_dict
        self.mudata.uns['ROMA_stats'] = pd.read_csv(os.path.join(path, "ROMA_stats.csv"), index_col=0)
        self.mudata.uns['ROMA_active_modules'] = pd.read_csv(os.path.join(path, "ROMA_active_modules.csv"), index_col=0)
    ...
    def get_module_scores(self, module_name: str) -> Optional[np.ndarray]:
        ...
        """
        if 'ROMA' not in self.mudata.uns:
            raise ValueError("No ROMA results found. Run compute() first.")
        
        if module_name not in self.mudata.uns['ROMA']:
            warnings.warn(f"Module '{module_name}' not found in results.")
            return None
        
        result = self.mudata.uns['ROMA'][module_name]
    ...
    def export_results(self, 
        ...
        """
        if 'ROMA_stats' not in self.mudata.uns:
            raise ValueError("No ROMA results found. Run compute() first.")
        
        stats_df = self.mudata.uns['ROMA_stats']
        ...
        if format == 'csv':
            ...
            with pd.ExcelWriter(output_file) as writer:
                ...
                if 'ROMA_active_modules' in self.mudata.uns:
                    self.mudata.uns['ROMA_active_modules'].to_excel(
                        writer, sheet_name='Active_Modules'
                    )
                
                if include_projections:
                    for module_name, result in self.mudata.uns['ROMA'].items():
            ...
            if 'ROMA_active_modules' in self.mudata.uns:
                self.mudata.uns['ROMA_active_modules'].to_hdf(
    ...
    def summary(self) -> None:
        ...
        stats = self.mudata.uns['ROMA_stats']
        active = self.mudata.uns.get('ROMA_active_modules', pd.DataFrame())
...
class ROMA_GSVD:
    ...
    genesets : Dict[str, np.ndarray]
        Dictionary mapping gene set names to gene arrays
    results : Dict[str, GeneSetResult_GSVD]
        Results for each analyzed gene set
    null_distributions : Dict[int, Tuple[np.ndarray, np.ndarray]]
        Cached null distributions by gene set size
    approx_int : int
    ...
    def __init__(self) -> None:
        ...
        self.global_gene_counts: Dict[str, int] = {}
        self.global_outlier_counts: Dict[str, int] = {}
        self.gsvd: Optional[Union[pygsvd]] = None
        self.X: Optional[np.ndarray] = None
        self.raw_X_subset: Optional[np.ndarray] = None
        self.nulll1: List[float] = []
        self.test_l1: Optional[float] = None
        ...
        self.test_median_exp_M1: Optional[float] = None
        self.test_median_exp_M2: Optional[float] = None
        self.med_exp_p_value_M1: Optional[float] = None
        self.med_exp_p_value_M2: Optional[float] = None
        self.projections_1: Optional[np.ndarray] = None
        self.projections_2: Optional[np.ndarray] = None
        self.results: Dict[str, GeneSetResult_GSVD] = {}
        self.null_distributions: Dict[int, Tuple[np.ndarray, np.ndarray]] = {}
        self.unprocessed_genesets: List[str] = []
        ...
        # Thresholds
        self.q_L1_threshold: float = 0.05
        self.q_Med_Exp_M1_threshold: float = 0.05
        self.q_Med_Exp_M2_threshold: float = 0.05
    ...
    def loocv(self, subset: sc.AnnData, verbose: int = 0, for_randomset: bool = False) -> List[int]:
        ...
        X = self.subset.X.T
        n_features, n_samples = X.shape
        X_df=anndata_to_multiindex_df(self.subset).T.copy() #to have the right type for GSVD
        X = np.asarray(X)
        if n_samples < 2:
            if verbose:
                print(f"Cannot perform LOOCV with {n_samples} samples.")
        ...
        for i, (train_index, _) in enumerate(loo.split(X)):
            ...
            vals=X_df.iloc[train_index]
            m1,m2=X_df.index.get_level_values(0).unique().to_list()
            M1=vals.loc[m1].to_numpy()
            M2=vals.loc[m2].to_numpy()
            D1, D2, Y, R, U, V = pygsvd.gsvd(M1, M2)
            gsvd=GSVD_wrapper(D1, D2, Y, R, U, V, n_components=1)
            
            l1scores[i] = gsvd.explained_variance_ratio_[0]
    ...
    def robustGSVD(self, 
                ...
                subsetlist: np.ndarray, 
                outliers: List[int], 
                for_randomset: bool = False ) -> Tuple[np.ndarray]:
        # Filter outliers to only include valid indices for current gene_subset
        outliers_array = np.array(outliers)
        valid_outliers = outliers_array[outliers_array < len(subsetlist)]
        ...
        # Extract data for robust gene subset
        X_df=anndata_to_multiindex_df(adata[:, robust_gene_subset]).copy()
            
        X_df = X_df.T  # Transpose to have genes as rows, samples as columns
        
        # Center the data
        #X_df = X_df - X_df.mean(axis=0)
        
        # Perform GSVD
        m1,m2=X_df.index.get_level_values(0).unique().to_list()
        M1,M2=[X_df.loc[m1].to_numpy(), X_df.loc[m2].to_numpy()] #[((X_df.T)["prot"].T).to_numpy(), ((X_df.T)["transcri"].T).to_numpy()] 
        D1,D2,Y,R,U,V  = pygsvd.gsvd(M1, M2) 

        gsvd_ = GSVD_wrapper(D1,D2,Y,R,U,V, n_components=2)
    ...
    def compute_median_exp(self, 
        ...
        projections_2 : np.ndarray
            Gene projections on PC2
        """
        pc1, pc2 = gsvd_.components_
        
        # Orient PC1
        correct_sign = self.orient_pc1(pc1, X, raw_X_subset, gene_set_name)
        self.correct_pc_sign = correct_sign ##WARNING: adding PC orientation #1.0 #
        pc1 = correct_sign * pc1
        
        # Compute projections
        m1,m2=list(self.adata.var["modality"].unique())
        projM1_1=X.loc[m1] @ pc1
        projM2_1=X.loc[m2] @ pc1
        
        projM1_2=X.loc[m1] @ pc2
        projM2_2=X.loc[m2] @ pc2

        projections_1 = pd.concat([projM1_1, projM2_1], axis=0, keys=[m1, m2])
        projections_2 = pd.concat([projM1_2, projM2_2], axis=0, keys=[m1, m2])
        ...
        # Compute median
        #median_exp = np.median(projections_1)
        #median_exp = np.mean([np.median(projA_1),np.mean(projB_1)])
        median_exp_M1 = np.median(projM1_1)
        median_exp_M2 = np.median(projM2_1)

        return median_exp_M1, median_exp_M2, projections_1, projections_2

    def process_iteration(self, 
                            sequence: np.ndarray, 
                            idx: np.ndarray, 
                            iteration: int) -> Tuple[float, float, np.ndarray, np.ndarray]:
        """
        Process single iteration for null distribution computation.
        ...
        gsvd_, X = self.robustGSVD(self.adata, gene_subset, outliers, 
                                        for_randomset=True)
        
        l1 = gsvd_.explained_variance_ratio_[0]
        
        # Get raw subset for median computation
        subsetlist = [x for i, x in enumerate(gene_subset) if i not in outliers]
        raw_X_subset = self.adata.raw[:, subsetlist].X.T
        
        median_exp_M1, median_exp_M2, null_projections_1, null_projections_2 = self.compute_median_exp(
            gsvd_, X, raw_X_subset
        )
        
        return l1, median_exp_M1, median_exp_M2, null_projections_1, null_projections_2
    ...
    def randomset_parallel(self, 
        ...
        # Check cache
        if candidate_nullgeneset_size in self.null_distributions:
            self.nulll1, self.null_median_exp_M1, self.null_median_exp_M2 = self.null_distributions[candidate_nullgeneset_size]
            if verbose:
                print('Using existing null distribution')
            return
        ...
        # Unpack and convert results
        nulll1, null_median_exp_M1, null_median_exp_M2, null_proj1, null_proj2 = zip(*results)
        
        nulll1_array = np.array(nulll1)
        null_median_exp_array_M1 = np.array(null_median_exp_M1)
        null_median_exp_array_M2 = np.array(null_median_exp_M2)
        
        # Store in cache
        self.null_distributions[candidate_nullgeneset_size] = [
            nulll1_array.reshape(-1, 1),  # Ensure 2D array
            null_median_exp_array_M1,
            null_median_exp_array_M2,
        ]
        
        self.nulll1 = nulll1_array.reshape(-1, 1)
        self.null_median_exp_M1 = null_median_exp_array_M1
        self.null_median_exp_M2 = null_median_exp_array_M2
    ...
    def get_raw_p_values(self, gene_set_name: Optional[str] = None) -> None:
        ...
        if not hasattr(self, 'nulll1') or self.nulll1 is None or len(self.nulll1) == 0:
            # No null distribution available, use default values
            test_l1 = self.gsvd.explained_variance_ratio_[0] 
            
            self.test_l1 = test_l1
            self.p_value = 0.5  # Default p-value when no null distribution
            
            # Compute median expression
            test_median_exp_M1, test_median_exp_M2, projections_1, projections_2 = self.compute_median_exp(
                self.gsvd, self.X, self.raw_X_subset, gene_set_name
            )
            
            self.test_median_exp_M1 = test_median_exp_M1
            self.test_median_exp_M2 = test_median_exp_M2
            self.med_exp_p_value = 0.5  # Default p-value
            self.projections_1 = projections_1
            self.projections_2 = projections_2
        ...
        null_median_distribution_M1 = np.array(self.null_median_exp_M1)
        null_median_distribution_M2 = np.array(self.null_median_exp_M2)
        ...
        # Empirical p-value
        med_exp_p_value_M1 = (np.sum(np.abs(null_median_distribution_M1) >= np.abs(test_median_exp_M1)) + 1) / (len(null_median_distribution_M1) + 1)
        med_exp_p_value_M2 = (np.sum(np.abs(null_median_distribution_M2) >= np.abs(test_median_exp_M2)) + 1) / (len(null_median_distribution_M2) + 1)
        
        self.test_median_exp_M1 = test_median_exp_M1
        self.test_median_exp_M2 = test_median_exp_M2
        self.med_exp_p_value_M1 = med_exp_p_value_M1
        self.med_exp_p_value_M2 = med_exp_p_value_M2
        self.projections_1 = projections_1
        self.projections_2 = projections_2
        
        # Clear temporary data
        self.nulll1 = None
        self.null_median_exp_M1 = None
        self.null_median_exp_M2 = None
        self.raw_X_subset = None

    def assess_significance(self, results: Dict[str, GeneSetResult_GSVD]) -> Dict[str, GeneSetResult_GSVD]:
        """
        Compute adjusted p-values using Benjamini-Hochberg procedure.
        ...
        n_tests = len(results)
        ps = np.zeros(n_tests)
        med_ps_M1 = np.zeros(n_tests)
        med_ps_M2 = np.zeros(n_tests)
        
        # Collect p-values
        for i, (_, gene_set_result) in enumerate(results.items()):
            ps[i] = gene_set_result.non_adj_p
            med_ps_M1[i] = gene_set_result.non_adj_med_exp_p_M1
            med_ps_M2[i] = gene_set_result.non_adj_med_exp_p_M2
        
        # Apply Benjamini-Hochberg correction
        qs = benj_hoch(ps)
        med_exp_qs_M1 = benj_hoch(med_ps_M1)
        med_exp_qs_M2 = benj_hoch(med_ps_M2)
        
        # Update results
        for i, (_, gene_set_result) in enumerate(results.items()):
            gene_set_result.q_value = qs[i]
            gene_set_result.med_exp_q_value_M1 = med_exp_qs_M1[i]
            gene_set_result.med_exp_q_value_M2 = med_exp_qs_M2[i]
    ...
    def p_values_in_frame(self, assessed_results: Dict[str, GeneSetResult_GSVD]) -> pd.DataFrame:
        """
        Create DataFrame with all p-values and statistics.
        
        Parameters
        ----------
        assessed_results : Dict[str, GeneSetResult_GSVD]
            Results with computed p-values
        ...
        for name, result in assessed_results.items():
            data['L1'][name] = result.test_l1
            data['ppv L1'][name] = result.non_adj_p
            data[f'Median Exp {m1}'][name] = result.test_median_exp_M1
            data[f'ppv Med Exp {m1}'][name] = result.non_adj_med_exp_p_M1
            data[f'Median Exp {m2}'][name] = result.test_median_exp_M2
            data[f'ppv Med Exp {m2}'][name] = result.non_adj_med_exp_p_M2
            data['q L1'][name] = result.q_value
            data[f'q Med Exp {m1}'][name] = result.med_exp_q_value_M1
            data[f'q Med Exp {m2}'][name] = result.med_exp_q_value_M2
    ...
    def compute(self, 
        ...
        for gene_set_name in sorted_gene_sets:
            ...
            # Perform decomposition
            self.robustGSVD(self.adata, self.subsetlist, self.outliers)
            ...
            # Store results
            gene_set_result = GeneSetResult_GSVD(
                self.subset, 
                self.subsetlist, 
                self.outliers, 
                self.nullgenesetsize,
            ...
            gene_set_result.custom_name = f"GeneSetResult_GSVD {gene_set_name}"
            gene_set_result.test_l1 = self.test_l1
            gene_set_result.non_adj_p = self.p_value
            gene_set_result.non_adj_med_exp_p_M1 = self.med_exp_p_value_M1
            gene_set_result.non_adj_med_exp_p_M2 = self.med_exp_p_value_M2
            gene_set_result.test_median_exp_M1 = self.test_median_exp_M1
            gene_set_result.test_median_exp_M2 = self.test_median_exp_M2
            gene_set_result.projections_1 = self.projections_1
            gene_set_result.projections_2 = self.projections_2
        ...
        self.mudata.uns['ROMA'] = assessed_results
        self.mudata.uns['ROMA_stats'] = self.p_values_in_frame(assessed_results)
        self.select_active_modules(self.q_L1_threshold, self.q_Med_Exp_M1_threshold, self.q_Med_Exp_M2_threshold)
    ...
    def select_active_modules(self, 
                            q_L1_threshold: float = 0.05, 
                            q_Med_Exp_M1_threshold: float = 0.05,
                            q_Med_Exp_M2_threshold: float = 0.05) -> None:
        """
        Select significantly active modules based on q-value thresholds.
        ...
        """
        df = self.mudata.uns['ROMA_stats']
        m1,m2=list(self.mudata.mod)#list(self.adata.var["modality"].unique())
        active_modules = df[(df['q L1'] <= q_L1_threshold) | (df[f'q Med Exp {m1}'] <= q_Med_Exp_M1_threshold) | (df[f'q Med Exp {m2}'] <= q_Med_Exp_M2_threshold)]
        self.mudata.uns['ROMA_active_modules'] = active_modules
    ...
    def save_active_modules_results(self, 
        ...
        attributes = {
            ...
            "projections_1": "numpy.ndarray",
            "projections_2": "numpy.ndarray",
            "gsvd.components_": "numpy.ndarray"
        }
        ...
        if save_mudata:
            del self.mudata.uns['ROMA']  # Remove large dictionary before saving
            self.mudata.write(os.path.join(path, "roma_mudata.h5mu"))                ##TODO
    ...
    def load_active_modules_results(self, path: str) -> None:
        ...
        for key in os.listdir(path):
            key_dir = os.path.join(path, key)
            if os.path.isdir(key_dir):
                gene_set = GeneSetResult_GSVD(None, None, None, None, None)
                gene_set.custom_name = key
                ...
                for file in os.listdir(key_dir):
                    ...
                    if attr_name == "subsetlist":
                        ...
                        gene_set.projections_2 = np.load(file_path, allow_pickle=True)
                    elif attr_name == "gsvd.components_":
                        components = np.load(file_path, allow_pickle=True)
                        gene_set.gsvd = SimpleNamespace(components_=components)
                
                loaded_dict[key] = gene_set
        
        # Load AnnData if not already loaded
        if self.mudata is None:
            mudata_path = os.path.join(path, "roma_mudata.h5mu")     ##TODO
            if os.path.exists(mudata_path):
                self.mudata = mu.read_h5mu(mudata_path)               ##TODO
            else:
                raise FileNotFoundError(f"MuData file not found: {mudata_path}")
    ...
    def get_module_scores(self, module_name: str) -> Optional[np.ndarray]:
        ...
        scores : Optional[np.ndarray]
            Activity scores across samples, or None if module not found
        """
        raise ValueError("This function hasn't been corrected yet!")
        ...
        result = self.mudata.uns['ROMA'][module_name]
        # Compute sample scores from GSVD
        if hasattr(result, 'gsvd') and result.gsvd is not None:
            # Sample scores are the projection of samples onto PC1
            pc1 = result.gsvd.components_[0]
            
            flat_adata = mudata_to_flat_anndata_v2(self.mdata, feature_col="modality")
            sample_scores = flat_adata[:, subsetlist].X @ result.projections_1 #sample_scores = self.adata[:, subsetlist].X @ result.projections_1
    ...
    def compare_conditions(self, 
        ...
        ----------
        condition_key : str
            Key in mudata.obs containing condition labels
        module_name : str
            Name of module to compare
        test : str, default='wilcoxon'
            Statistical test to use ('wilcoxon', 't-test', 'mann-whitney')
        ...
        results : Dict[str, Any]
            Test results including p-value and effect size
        ###
        raise ValueError("get_module_scores function hasn't been corrected yet")
        from scipy.stats import mannwhitneyu, ttest_ind
        ...
        if condition_key not in self.mudata.obsm:
            raise ValueError(f"Condition key '{condition_key}' not found in mudata.obsm.")
        
        conditions = self.mudata.obsm[condition_key].unique()
        if len(conditions) != 2:
            raise ValueError(f"Expected 2 conditions, found {len(conditions)}.")
        
        # Split scores by condition
        mask1 = self.mudata.obsm[condition_key] == conditions[0]
        mask2 = self.mudata.obsm[condition_key] == conditions[1]
    ...
    def summary(self) -> None:
        ...
        print(f"Total gene sets analyzed: {len(stats)}")
        print(f"Active modules (overdispersed): {len(active[active['q L1'] <= self.q_L1_threshold])}")
        print(f"Active modules (shifted in {m1}): {len(active[active[f'q Med Exp {m1}'] <= self.q_Med_Exp_M1_threshold])}")
        print(f"Active modules (shifted in {m2}): {len(active[active[f'q Med Exp {m2}'] <= self.q_Med_Exp_M2_threshold])}")
        ...
        # Top modules by median expression
        print(f"\n{color.BOLD}Top 5 shifted modules for {m1} :{color.END}")
        top_med = stats.nsmallest(5, f'q Med Exp {m1}')[[f'Median Exp {m1}', f'q Med Exp {m1}']]
        for idx, row in top_med.iterrows():
            print(f"  {idx}: Med={row[f'Median Exp {m1}']:.3f}, q={row[f'q Med Exp {m1}']:.3e}")

        # Top modules by median expression
        print(f"\n{color.BOLD}Top 5 shifted modules for {m2} :{color.END}")
        top_med = stats.nsmallest(5, f'q Med Exp {m2}')[[f'Median Exp {m2}', f'q Med Exp {m2}']]
        for idx, row in top_med.iterrows():
            print(f"  {idx}: Med={row[f'Median Exp {m2}']:.3f}, q={row[f'q Med Exp {m2}']:.3e}")
